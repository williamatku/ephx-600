    ...
    for trial_no in range(TRIALS):
        for n_qubits in range(2, MAX_QUBITS+1):
            for enc_freqs in TEST_FREQS:
                ...
                print(f"trial no: {trial_no}, n_qubits: {n_qubits}, enc_freqs: {enc_freqs}, score: {score}, average confidence: {conf}")
                scoresheet.append([trial_no, n_qubits, enc_freqs, score, conf])
